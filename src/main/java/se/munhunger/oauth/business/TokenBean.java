		...
		try
		{
			...
			if(validClientCredentials)
			{
				...
				if(!tokenList.isEmpty())
				{
					...
					params = new HashMap<>();
					params.put("username", authToken.username);
					User user = (User) Database.getObjects("from User WHERE username = :username", params).get(0);
					...
					String jwt = JSONWebToken.buildToken(client.jwtKey, accessToken,
							...
							user.getRolesCSV() + ";authenticated", clientID, user.username, user.number,
							72000000);
